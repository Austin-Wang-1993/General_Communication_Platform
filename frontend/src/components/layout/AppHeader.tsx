 ...
 */
export function AppHeader({ title, backTo, backLabel = '返回', end, below }: AppHeaderProps) {
  return (
    <header className="sticky top-0 z-40 border-b border-border-subtle bg-paper/95 backdrop-blur supports-[padding:max(0px)]:pt-[max(12px,env(safe-area-inset-top))]">
      ...
      {below && <div className="border-t border-border-subtle px-2 pb-2 pt-1">{below}</div>}
    </header>
  );
}
