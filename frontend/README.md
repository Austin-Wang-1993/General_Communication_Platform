...
- **P3** `/scenarios/:id/chat`：`GET runtime`、`POST enter`、`POST turns`；首入若运行指针未设置则**自动**按 framework **S[0]** `enter` 再拉 `runtime`（**F-P3-00**）；顶栏 **返回首页 / 背景介绍 / 回答提示（R1）/ 总结分析（R2）/ 查看列表**；气泡、**选择信息接收人**、P3a 进节
