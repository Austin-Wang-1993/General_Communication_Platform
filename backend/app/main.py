...
M0~M5：health、场景包 CRUD、commit-intake、framework/world Job、轮询与取消、运行期 runtime + 进节 enter。
"""
...
from app.routers import creation_jobs, health, runtime, scenario_packages
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="GCP Backend",
        description="通用英语对话练习 · 后端 API（M0~M5）",
        version=__version__,
        lifespan=lifespan,
    )
