...
if _gcp_backend_unit_ready; then
    ...
    if curl -fsS --max-time 15 "${LOCAL_OPENAPI_URL}" 2>/dev/null | grep -qE "scenario-packages|commit-intake|jobs/framework|jobs/world|jobs/.*/cancel|/runtime|/enter|/turns|auto-opener|/hints|/analytics|raw-file"; then
