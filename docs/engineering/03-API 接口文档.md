...
> **文档版本**：v0.1.15  
> **更新时间**：2026-05-15  
> **关联**：`../product/01-中台产品文档.md`（中台 v0.5.14）、`../product/02-前端需求文档.md`（v0.2.15）、`01-技术方案.md`（v0.3.15）、`02-代码架构与目录约定.md`（v0.1.13）  
...
### 4.6 POST `/api/v1/translation/en-to-zh`

**作用**：P3 **NPC 气泡**或 **工具弹窗**（背景介绍 / R1 回答提示 / R2 总结分析）标题栏「翻」——将调用方提交的英文正文（可为**多块合订**，见 **中台 §6.9.1**）转为**简体中文**展示；**不**写入 `turns.jsonl`，**不**改变 R1/R2 持久化对象与运行态（中台 **§6.9**）。

**关联中台**：§6.9.1～§6.9.4。

**前置条件**：无（首版**不**校验 `scenario_id` / 运行指针；若未来要审计可再扩展）。

**请求体**：

```json
{
  "text": "Alex, let's move the stand-up forward by fifteen minutes."
}
```

| 字段 | 类型 | 必填 | 约束 |
|---|---|---|---|
| `text` | string | 是 | 1～8000 字符 UTF-8；trim 后须非空 |

**成功响应 200**：

```json
{
  "translated_text": "亚历克斯，我们把站会提前十五分钟吧。"
}
```

| 字段 | 说明 |
|---|---|
| `translated_text` | 简体中文译文；自然、忠实于原意 |

**错误响应**：

| 状态 | error_code |
|---|---|
| 422 | `content_empty_or_too_long`（全空白或超长） |
| 401 / 403 / 500 / 504 | 与 §0.7 统一错误体一致；上游 LLM 失败时为 `llm_authentication_error` / `llm_failure` / `llm_timeout` |
...
| P3-11-01「翻」（NPC 气泡） | `POST /translation/en-to-zh` |
| P3-12-01～03「翻」（三工具弹窗） | `POST /translation/en-to-zh`（多块合订为一条 `text`） |
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v0.1.15 | 2026-05-15 | **§4.6** 补充：同一接口承载弹窗**多块英文合订**翻译（中台 **§6.9.1**、前端 **F-P3-12**）；头部关联版本号同步 |
| v0.1.14 | 2026-05-14 | **§4.6** 新增 `POST /translation/en-to-zh`（中台 **§6.9**、前端 **F-P3-11**）；§9 追溯表补 **P3-11-01**；头部关联版本号同步 |
| v0.1.13 | 2026-05-14 | 头部「关联」版本号：中台 v0.5.12、前端 v0.2.13、技术方案 v0.3.13、代码架构 v0.1.12 |
| v0.1.12 | 2026-05-14 | 头部「关联」版本号：中台 v0.5.11、前端 v0.2.12、技术方案 v0.3.12、代码架构 v0.1.11 |
| v0.1.11 | 2026-05-14 | 头部「关联」版本号：中台 v0.5.10、前端 v0.2.11、技术方案 v0.3.11、代码架构 v0.1.10 |
